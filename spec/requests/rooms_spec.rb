...
RSpec.describe 'Rooms API', type: :request do
  ...
  describe 'POST /rooms' do
    # valid payload
    let(:valid_attributes) { { room_name: 'Riverhouse', charge: 100 } }

    context 'when the request is valid' do
      before { post '/rooms', params: valid_attributes }

      it 'creates a room' do
        expect(json['title']).to eq('Riverhouse')
      end
    ...
    context 'when the request is invalid' do
      before { post '/rooms', params: { room_name: 'Foobar' } }
  ...
  describe 'PUT /rooms/:id' do
    let(:valid_attributes) { { room_name: 'Shopping' } }
